...
// またC言語にはもともとatoiという文字列を数値に変換する関数が用意されています。(stdlib.hを読み込むと使える)
// 今回作成したatoiではマイナスの値に対応していないなど、不完全な部分もありテストも充分ではありません。
// 勉強する上で標準の関数を模倣するのは力になりますが、実際の仕事ではすでに用意されていて
// 機能面、実績面で信頼があるモノを使うようにしましょう。
